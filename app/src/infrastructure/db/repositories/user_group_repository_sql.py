...
class SQLUserGroupRepository(UserGroupRepository):
    ...
    def select_user_groups(self, cursor: int | None, limit: int) -> list[UserGroup]:
        statement = select(UserGroupModel).order_by(UserGroupModel.id)
        if cursor:
            statement = statement.where(UserGroupModel.id >= cursor)
    ...
    def get_user_group_by_position(self, position: int) -> UserGroup | None:
        if position >= 0:
            statement = select(UserGroupModel).order_by(UserGroupModel.id.asc()).offset(position).limit(1)
        else:
            statement = select(UserGroupModel).order_by(UserGroupModel.id.desc()).offset(abs(position) - 1).limit(1)
    ...
    def select_user_group_by_id(self, user_group_id: int) -> UserGroup:
        user_group_model = self.session.get(UserGroupModel, user_group_id)
        if not user_group_model:
            raise NotFoundError("UserGroup", user_group_id)

        return UserGroup(**user_group_model.model_dump())
